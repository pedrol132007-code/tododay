...
- Convite converte em membro **quando o e-mail é confirmado** (não no cadastro), para ninguém herdar acesso cadastrando o e-mail de outra pessoa. Se o convidado já tem conta, entra na hora.
- Não é possível remover/rebaixar o último admin de uma equipe.
- SMTP: Gmail dedicado com senha de app.
- Dois projetos Supabase na mesma organização: `tododay-dev` e `tododay-prod`. A chave `service_role` nunca vai para o frontend; só a Edge Function a usa.
...
- `team_invite(id, team_id, email, role, job_title, invited_by, created_at, accepted_at, revoked_at)` — um pendente por equipe+e-mail
...
| E6 | Tela da equipe, convites (Edge Function `invite-member`), cargo, SMTP Gmail no dev | Contas `+teste`: sem equipe → convidado entra; e-mail novo → criar senha → boards; membro não convida |
...
| E11 | Deploy Vercel + Supabase prod, `vercel.json` (SPA), Site URL/Redirect URLs, templates em PT | Janela anônima: convite com e-mail novo → criar senha → ver o board |
